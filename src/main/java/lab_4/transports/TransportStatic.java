...
public class TransportStatic {
    ...
    public static double calcMidPrice(Transport transport) {
        double[] transportPrices = transport.getAllModelPrices();
        double sum = 0;
        for (double v : transportPrices) {
            sum += v;
        }
        return sum / transportPrices.length;
    }

    public static double calcMidPrices(Transport... transports) {
        double sum = 0;
        for (Transport veh : transports) {
            sum += calcMidPrice(veh);
        }
